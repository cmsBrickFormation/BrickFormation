...
public class ArcadeBrickP1 : MonoBehaviour
{
    ...
    void checkInput() {
        ...
        if (Input.GetKeyDown(KeyCode.S) && isValidMove(0, -moveVal, 0)) move(0, -moveVal);
    }
    ...
    void fallDown() {
        if (Time.time - fallTime >= fallSpeed) {
            if (isValidMove(0, -moveVal, 0)) {
                ...
                if (!FindObjectOfType<ArcadeGridP1>().isGameOver) {
    ...
    void move(int x, int y) {
        ...
        FindObjectOfType<ArcadeGridP1>().updateGrid(this.gameObject);
    }
    ...
    void rotate(int z, bool isActuallyARotation) {
        ...
        if (isActuallyARotation) FindObjectOfType<ArcadeGridP1>().updateGrid(this.gameObject);                                                  // this is needed because the brick is also rotated when validating a move
    }
    ...
    bool isValidMove(int moveX, int moveY, int rotateZ) {
        ...
        foreach (Transform cube in this.gameObject.transform) {
            ...
            if (FindObjectOfType<ArcadeGridP1>().isInsideGrid(attemptedPos) == false) isValid = false;                                              // if a cube is out of bounds then the move is invalid
            else if (FindObjectOfType<ArcadeGridP1>().getTransformAtGridPosition(attemptedPos) != null)                                             // also, if it's in bounds but there is already a brick on that spot
                if (FindObjectOfType<ArcadeGridP1>().getTransformAtGridPosition(attemptedPos).parent != this.gameObject.transform) isValid = false; // and if that brick is not the one we want to move, then the move is also invalid
        }
