...
public class TowerPlatformP1 : MonoBehaviour
{
    public static int platformP1Width = 20;
    ...
    void Update() {
        if (PlayerPrefs.GetFloat("livesp1") <= 0 || PlayerPrefs.GetInt("winnerp1") == 1) isGameOver = true;
        if (isGameOver) {
    ...
    public bool isInsideBounds(Vector3 pos) {
        return (pos.x >= 0 && pos.x < platformP1Width && pos.y >= 5);
    }
