...
public class TowerPlatformP4 : MonoBehaviour
{
    public static int platformP4Width = 20;
    ...
    void Update() {
        if (PlayerPrefs.GetFloat("livesp4") <= 0 || PlayerPrefs.GetInt("winnerp4") == 1) isGameOver = true;
        if (isGameOver) {
            Destroy(FindObjectOfType<UtilityBrickPreviewP4>().gameObject);
            Destroy(livesText);
    ...
    public bool isInsideBounds(Vector3 pos) {
        return (pos.x >= 0 && pos.x < platformP4Width && pos.y >= 5);
    }
    ...
    public void instantiateNextBrick() {
        ...
        FindObjectOfType<UtilityBrickPreviewP4>().updateBrickPreview(rngIndex);
    }
}
