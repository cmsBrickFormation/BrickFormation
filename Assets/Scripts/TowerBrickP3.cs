...
public class TowerBrickP3 : MonoBehaviour
{
    ...
    void fallDown() {
        if (Time.time - fallTime >= fallSpeed) {
            if (isValidMove(0, -moveVal, 0)) {
                ...
                enabled = false;
                FindObjectOfType<TowerPlatformP3>().updateRows();
                FindObjectOfType<TowerPlatformP3>().checkGameOver(this.gameObject);
                if (!FindObjectOfType<TowerPlatformP3>().isGameOver) {
    ...
    void move(int x, int y) {
        ...
        FindObjectOfType<TowerPlatformP3>().updateGrid(this.gameObject);
    }
    ...
    void rotate(int z, bool isActuallyARotation) {
        ...
        if (isActuallyARotation) FindObjectOfType<TowerPlatformP3>().updateGrid(this.gameObject);                                                  // this is needed because the brick is also rotated when validating a move
    }
    ...
    bool isValidMove(int moveX, int moveY, int rotateZ) {
        ...
        foreach (Transform cube in this.gameObject.transform) {
            ...
            if (FindObjectOfType<TowerPlatformP3>().isInsideGrid(attemptedPos) == false) isValid = false;                                              // if a cube is out of bounds then the move is invalid
            else if (FindObjectOfType<TowerPlatformP3>().getTransformAtGridPosition(attemptedPos) != null)                                             // also, if it's in bounds but there is already a brick on that spot
                if (FindObjectOfType<TowerPlatformP3>().getTransformAtGridPosition(attemptedPos).parent != this.gameObject.transform) isValid = false; // and if that brick is not the one we want to move, then the move is also invalid
        }
