...
public class TowerPlatformP3 : MonoBehaviour
{
    public static int platformP3Width = 20;
    ...
    void Update() {
        if (PlayerPrefs.GetFloat("livesp3") <= 0 || PlayerPrefs.GetInt("winnerp3") == 1) isGameOver = true;
        if (isGameOver) {
    ...
    public bool isInsideBounds(Vector3 pos) {
        return (pos.x >= 0 && pos.x < platformP3Width && pos.y >= 5);
    }
